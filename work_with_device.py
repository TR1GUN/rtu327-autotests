...
def get_crc(request):
    ...
    # try:
    #     return bytes.fromhex(hex(res)[2:])
    # except Exception as ex: #Спереди надо подставить 0, чтобы число стало hex пример 3da => 03da => \x03\xda
    #     # Или это не правильно ?
    #     return bytes.fromhex('0' + hex(res)[2:])
...
def get_number_of_request(request):
    # result_request_array = str(request)[2:-1].split(r'\x')[1:] ## Первый пустой ? ##Сделать методом ?

    result_request_array = divide_bytes_string_per_byte(request) ## Первый пустой ? ##Сделать методом ?
    amount_of_bytes_in_request = 0
    for _ in result_request_array:
        if _[0] == 'x' and len(_) > 2 : _ = _[1:] ## Проверяем x00
        if len(_) == 2:
            amount_of_bytes_in_request += 1
        elif len(_) <= 1:
            raise Exception("Can't be 1 symbol")
        else:
            amount_of_bytes_in_request += len(_) - 1
    number_of_bytes = hex(amount_of_bytes_in_request)[2:]
...
def send_command_and_get_answer(command_number=None,command_params=b'', send_command_raw=None):
    ...
    # print(res.sendall(b'\x02\x00\x0d\x01\x00\x00\x00\x00\x00\x00\x00s9\x00\x00\x00\xce\x10'))
